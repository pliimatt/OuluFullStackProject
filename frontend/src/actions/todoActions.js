...
export const CLEAR_TODO_STATE = "CLEAR_TODO_STATE";
...
export const getList = (token) => {
	return async (dispatch) => {
		...
		dispatch(loading());
		let response = await fetch("/api/todos", request).catch(error => console.log(error))
...
export const removeFromList = (token, id) => {
	return async (dispatch) => {
		...
		dispatch(loading());
		const response = await fetch("/api/todos/" + id, request).catch(error => console.log(error))
		dispatch(stopLoading());
		if (!response) {
...
export const editItem = (token, item) => {
	return async (dispatch) => {
		...
		dispatch(loading());
		const response = await fetch("/api/todos/" + item._id, request).catch(error => console.log(error))
		dispatch(stopLoading());
		if (!response) {
